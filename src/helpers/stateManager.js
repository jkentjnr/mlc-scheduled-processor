...
export default class stateManager {
    ...
    static async getPayloadFromDataStore(logger, executionKey) {
        ...
        try {
            ...
            payloadResponse = await s3Manager.getObject(null, s3bucket, payloadKey);
        }
        catch (e) {
    ...
    static async persistPayloadToDataStore(logger, state) {
        ...
        try { await s3Manager.putObject(s3bucket, `state/${state.executionKey}`, JSON.stringify(payload)); }
